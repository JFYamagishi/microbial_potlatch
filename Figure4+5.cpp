...
int main(int argc, char *argv[]){
	...
	char output4[256] = "Output4.txt";
	char output5A[256] = "Output5.txt";
	ofstream of;
	of.open(output5A, ios::trunc);
	of.close();
	of.open(output4, ios::trunc);
	...
	for(int index = 0; index < sampleSize; index++){
		...
		while(InvasionSpecies.size() < SpeNumMaking){
			Cell cellTemp(1.0);
		...
		for(int u1=0; u1 < SpeNum; u1++) InvasionSpecies[u1].id = u1;
		...
		for(int u1 = nt; u1 < n; u1++) moreFromStrongestCell += Ecosystem[0].Difs[u1];
		int eraseSpeciesNum;
		...
		of << endl << endl << "#最強のCell:番号" <<SpeNum-1<< "; 透過性がゼロの場合の、単離時の成長率λ=" <<Ecosystem[0].mu;
		of << endl << "#単離時の最適拡散係数:";
		for(int i = 0; i < n; i++) of << Ecosystem[0].Difs[i] << " ";
		of << endl;

		for(int alpha = InvasionSpecies.size()-1; alpha >= SpeNum; alpha--) InvasionSpecies.erase(InvasionSpecies.begin() + alpha);
		for(int alpha = 0; alpha < InvasionSpecies.size(); alpha++) of << endl << "#番号" << InvasionSpecies[alpha].id << "; 単離時の成長率λ=" << InvasionSpecies[alpha].mu << "; τ=" <<InvasionSpecies[alpha].tau;
		...
		// Figure 5
		if(Ecosystem.size() == 5 ||Ecosystem.size() == 6){
			vector<double> Leakages_from_Cells = vector<double>(Ecosystem.size(), 0.0);
			for(int alpha = 0; alpha < Ecosystem.size(); alpha++){
				for(int i = nt; i < n; i++){
					if(Ecosystem[alpha].Difs[i] > 0.0){
						if(Ecosystem[alpha].x[i] > xEnv[i]) Leakages_from_Cells[alpha] += Ecosystem[alpha].Difs[i]*(Ecosystem[alpha].x[i] - xEnv[i]);
					}
				}
			}
			vector<double> Leaked_Chems = vector<double>(n, 0.0);
			for(int i = nt + Nenzyme; i < n; i++){
				for(int alpha = 0; alpha < Ecosystem.size(); alpha++){
					if(Ecosystem[alpha].Difs[i] > 0.0){
						if(Ecosystem[alpha].x[i] > xEnv[i]) Leaked_Chems[i] += Ecosystem[alpha].Difs[i]*(Ecosystem[alpha].x[i] - xEnv[i]);
					}
				}
			}

			double totalLeakage = 0.0;
			for(int alpha = 0; alpha < Ecosystem.size(); alpha++) totalLeakage += Leakages_from_Cells[alpha];

			vector<int> survivalSpeciesNums = vector<int>(Ecosystem.size(), 0);

			vector<Cell> EcosystemOri = Ecosystem;
			for(int alpha = 0; alpha < EcosystemOri.size(); alpha++){
				Ecosystem.clear();
				Ecosystem = EcosystemOri;
				Ecosystem.erase(Ecosystem.begin() + alpha);
				double popFreqSum = 0.0;
				for(int beta = 0; beta < Ecosystem.size(); beta++) popFreqSum += Ecosystem[beta].popFreq;
				for(int beta = 0; beta < Ecosystem.size(); beta++) Ecosystem[beta].popFreq /= popFreqSum;

				for(double t = 0.0; t < T; t+=dt) PopDynamics();

				survivalSpeciesNums.push_back(Ecosystem.size());
			}
			of.close();
			of.open(output5A, ios::app);
			of << "# Figure 5A.";
			of << endl << "# Number of Coexisting Species: " << Ecosystem.size();
			of << endl << "# Removed Species ID, Number of Extinct Species, Ratio of Total Leakage";
			for(int alpha = 0; alpha < EcosystemOri.size(); alpha++){
				of << endl << EcosystemOri[alpha].id << " ";
				of << EcosystemOri.size() - 1 - survivalSpeciesNums[alpha] << " " << Leakages_from_Cells[alpha] / totalLeakage;
			}

			of << endl << endl << "#Figure 5B.";
			of << endl << "# S_Cell, S_Chem, Average Survival Ratio";
			double Scell = 0.0;
			for(int alpha = 0; alpha < EcosystemOri.size(); alpha++){
				if(Leakages_from_Cells[alpha] > 0.0){
					double prob = Leakages_from_Cells[alpha] / totalLeakage;
					Scell += prob * log(prob);
				}
			}
			double Schem = 0.0;
			for(int i = nt + Nenzyme; i < n; i++){
				if(Leaked_Chems[i] > 0.0){
					double prob = Leaked_Chems[i] / totalLeakage;
					Schem += prob * log(prob);
				}
			}
			double aveSurvivalRatio = 0.0;
			for(int alpha = 0; alpha < EcosystemOri.size(); alpha++) aveSurvivalRatio += 1.0 * survivalSpeciesNums[alpha];
			aveSurvivalRatio /= ( 1.0 * (EcosystemOri.size() - 1) );
			of << endl << Scell << " " << Schem << " " << aveSurvivalRatio;

			of << endl << endl << endl;
			of.close();
			of.open(output4, ios::app);
		}
	}
	
	of << endl << "# Number of Coexisting Species, Frequency";
	for(int i = 1; i < 9; i++){
		int CoexNumFreq = 0;
		for(int u1 = 0; u1 < SymGrowthRates.size(); u1++) if(SymGrowthRates[u1].first == i) CoexNumFreq++;
		of << endl << i << " " << CoexNumFreq;
	}
	of << "# Total Samples = " <<SymGrowthRates.size();
	of.close();
